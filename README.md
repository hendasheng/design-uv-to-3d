...
- 支持在 UV 图上叠加当前模型的 UV 线框，并可开关显示
- 支持自定义 UV 线框颜色
- 支持模型加载中和加载失败状态提示，避免 3D 区空白
...
- 颜色、金属、凹凸和透明贴图会优先作用在模型主材质上，透明玻璃等结构材质不会被强行覆盖。
- 右侧可切换当前模型关联的 UV 参考图，并可显示/隐藏 UV 线框、调整线框颜色，用于对照图案位置。

## 注意事项

- 建议压缩 GLB 和贴图资源，避免浏览器加载过慢。
- 页面首屏不会预加载全部 GLB，切换模型时会按需加载对应文件；局域网较慢或模型较大时，3D 区会显示加载提示。
- 当开启 UV 线框时，加载流程会包含 UV 线框生成；大模型首次生成 UV 线框会更慢，但结果会按模型缓存。
